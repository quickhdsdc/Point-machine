...
def gru_autoencoder(data_train=None, data_valid=None, time_steps=None, mask_val=0):
    ...
    states = Dense(70, activation='elu')(encoder_outputs)
    ...
    decoder = GRU(70, return_sequences=True, return_state=True)
    ...
    model.fit([data_train, data_train_reverse,
               decoder_input_data], data_train, batch_size=len(data_train),
...
#    index = [ind for ind, item in enumerate(ts_original) if len(item) > 150 and len(item) < 160]
    ...
    ts = [item for ind, item in enumerate(ts_original)]
    ...
    paddingTs = pad_sequences(ts, padding="pre", dtype="float64", value=-10)
    paddingTs[np.isnan(paddingTs)] = -10
    trainPrecent = 0.8
...
#    # Calculating the mask loss
#    mask_val = -10
#    y_true = np.array([1, 2, -10])
#    y_pred = np.array([1, 2, -10])
...
#    loss = K.abs(y_true - y_pred) * mask
    ...
    predicted_res, rep = gru_autoencoder(data_train=data_train, data_valid=data_valid,
                                         time_steps=tsTotalPtsNums, mask_val=-10)
#    
